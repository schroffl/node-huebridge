...
module.exports = debug => {
	...
	if(debug) {
		router.use(morgan(':remote-addr -- :method :url :status :response-time ms - :res[content-length]'));
	...
	/*router.use('/api/:username', (req, res, next) => {
		let requiresAuth = ![ 
				req.path === '/config',
				req.params.username === 'config' && req.path === '/' 
			].reduce((result, current) => result || current, false);

		if(requiresAuth)
			emitter.emit('authentication', req.params.username, next, module.exports.error.bind(null, req, res, 1));
		else
			res.json({ 'auth-required': false });
	});*/
